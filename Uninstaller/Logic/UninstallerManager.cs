            ...
            using (RegistryKey parent = Registry.LocalMachine.OpenSubKey(UninstallRegKeyPath, true))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        key.SetValue("InstallLocation", Uninstaller.Instance.InstallLocation);
                        key.SetValue("UninstallString", exe + " /uninstallprompt"); // TO be changed when uninstaller exe is deployed
